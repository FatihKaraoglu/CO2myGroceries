...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    public String getProdcut_IDForSpinner(String productName){
        String query = "select PRODUKT_ID FROM DATENBANK WHERE NAME__DEUTSCH_ = " + "\"" + productName + "\"";
        Cursor cursor = sqLiteDatabase.rawQuery(query, null);
        cursor.moveToFirst();
        String productID = cursor.getString(0);
        return productID;
    ...
    public String[] getAllNames(){
        String query = "select NAME__DEUTSCH_ from MARKEN";
        Cursor cursor = sqLiteDatabase.rawQuery(query, null);
        cursor.moveToFirst();
        String[] all_Products = new String[307];
        for(int i = 0; i < all_Products.length; i++){
            all_Products[i] = cursor.getString(0);
            cursor.moveToNext();
        }
        return all_Products;
    }
